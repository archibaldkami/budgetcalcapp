...
class Database {
    ...
    public:
        ...
        int insert_operation(int date, double amount, int category, const char* description = "") {
            ...
            sqlite3_stmt* stmt;
            const char* sql = "INSERT INTO op_opearation_history (date, amount, category, description) VALUES (?, ?, ?, ?);";
            int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr);
        ...
        vector<Operation> get_operations_by_vault(int id) {
        ...
        vector<Operation> get_operations_by_category(int id) {
            vector<Operation> result;
            char* errMsg = nullptr;
            char sql[128    ];
        ...
        vector<Category> get_categories(int id) {
        ...
        void fix_diff(int curr_category_id, double curr_amount, Operation prev_operation) {
            ...
            int curr_mod = curr_category.type-1 ? -1 : 1; // тип операції :P
            int prev_mod = prev_category.type-1 ? -1 : 1; // тип операції :P
            double bal = get_balance(curr_category.vault);
            bal += curr_amount * curr_mod - prev_operation.amount * prev_mod; // Розрахунок різниці операцій і нового поточного балансу 
        ...
        int update_operation(int id, int date, double amount, int category, char* description) {
            ...
            cout << green << "LOG " << reset << "update_operation success, ID: " << sqlite3_last_insert_rowid(db) << endl; // LOG
            sqlite3_finalize(stmt);
        ...
        int update_category(int id, const char* name, const char* color, const char* description) {
            ...
            cout << green << "LOG " << reset << "update_category success, ID: " << sqlite3_last_insert_rowid(db) << endl; // LOG
            sqlite3_finalize(stmt);
        ...
        int update_vault(int id, const char* name, const char* description) {
            ...
            cout << green << "LOG " << reset << "update_vault success, ID: " << sqlite3_last_insert_rowid(db) << endl; // LOG
            sqlite3_finalize(stmt);
        ...
        Database() {
            ...
            if (!file) {
                cout << green << "LOG " << reset << "Config is not found. Creating..." << endl; // LOG
                ...
                cout << green << "LOG " << reset << "Base config creation success" << endl; // LOG
            }
